...
TEST(TrajectoryTest, MC_TargetsMatchOutcome) {
    ...
    int n = extract_training_samples(game, TDMode::kMC, 0.0, samples, 4);
...
TEST(TrajectoryTest, TD0_Bootstrap) {
    ...
    TrainingSample samples[4];
    int n = extract_training_samples(game, TDMode::kTD0, 0.0, samples, 4);
...
TEST(TrajectoryTest, TDLambda_Lambda0_EqualsTD0) {
    ...
    extract_training_samples(game, TDMode::kTD0, 0.0, td0_samples, 4);
    extract_training_samples(game, TDMode::kTDLambda, 0.0, tdl_samples, 4);
...
TEST(TrajectoryTest, TDLambda_Lambda1_EqualsMC) {
    ...
    extract_training_samples(game, TDMode::kMC, 1.0, mc_samples, 4);
    extract_training_samples(game, TDMode::kTDLambda, 1.0, tdl_samples, 4);
...
TEST(TrajectoryTest, TDLambda_Half_BetweenTD0AndMC) {
    ...
    extract_training_samples(game, TDMode::kTD0,      0.0, td0, 6);
    extract_training_samples(game, TDMode::kMC,       0.0, mc,  6);
    extract_training_samples(game, TDMode::kTDLambda, 0.5, tdl, 6);
...
TEST(TrajectoryTest, AllTargets_InUnitRange) {
    ...
    for (auto mode : {TDMode::kTD0, TDMode::kMC}) {
        extract_training_samples(game, mode, 0.0, samples, 10);
        for (int i = 0; i < 10; ++i) {
            EXPECT_GE(samples[i].target, 0.0) << "mode=" << (int)mode << " step=" << i;
            EXPECT_LE(samples[i].target, 1.0) << "mode=" << (int)mode << " step=" << i;
        }
    }
    extract_training_samples(game, TDMode::kTDLambda, 0.7, samples, 10);
    for (int i = 0; i < 10; ++i) {
...
TEST(TrajectoryTest, Tensor_CopiedToSample) {
    ...
    extract_training_samples(game, TDMode::kMC, 0.0, samples, 3);
...
TEST(TrajectoryTest, MaxSamples_Respected) {
    ...
    int n = extract_training_samples(game, TDMode::kMC, 0.0, samples, 5);
    EXPECT_EQ(n, 5);
}
